#!/usr/bin/env python3
"""Safely check, preview, or install codebase-convergence to an explicit target."""
...
SOURCE = PROJECT_ROOT / "codebase-convergence"
SKILL_NAME = "codebase-convergence"
MANIFEST_NAME = ".codebase-convergence-install.json"
...
def load_manifest(target: Path) -> dict[str, Any] | None:
    ...
    if data.get("skill") != SKILL_NAME or not isinstance(data.get("files"), dict):
